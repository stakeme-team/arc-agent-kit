...
That's it! Claude Code automatically detects `.mcp.json` and connects to the Arc MCP server.
...
```
/wallet    — Create wallet (if missing) and show address + balance
...
"Send 0.001 USDC to a random address"
...
Inside Claude Code, run `/mcp` to see connected MCP servers. You should see `arc` listed with its tools.
