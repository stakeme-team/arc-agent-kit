...
function existingWalletAddress(): string | null {
...
async function generateSimple(): Promise<void> {
  const existing = existingWalletAddress();
  if (existing) {
    const proceed = await promptYesNo(
      `\u26a0  Wallet already exists: ${existing}\n   Generate a new one and overwrite .env? [y/N]: `
    );
    if (!proceed) {
      console.log("Aborted. Existing wallet preserved.");
      return;
    }
  }
...
async function generateSecure(): Promise<void> {
  ...
  if (existing) {
    const proceed = await promptYesNo(
      `\u26a0  Wallet already exists: ${existing}\n   Generate a new one and overwrite keystore + .env? [y/N]: `
    );
    if (!proceed) {
      console.log("Aborted. Existing wallet preserved.");
      return;
...
if (command === "generate") {
  ...
  console.log(`Arc Wallet Manager
